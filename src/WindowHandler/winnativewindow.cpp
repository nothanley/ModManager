...
void SetRoundWindowBorders(HWND hWnd,int width, int height, int radius)
{
    // Create a rounded region
    HRGN roundRegion = CreateRoundRectRgn(0, 0, width, height, radius);

    // Set the window's region to the rounded region
    SetWindowRgn(hWnd, roundRegion, TRUE);
    ...
    : hWnd(nullptr)
{
    ...
    // Ensure the window does not have a menu
    SetMenu(hWnd, NULL);
}
